...
int solveUsingRecursion(int weight[], int value[], int index, int capacity) {
  ...
  if(index == 0 ) {
    if(weight[0] <= capacity) {  //last element can be fit into the bag so add its value
      return value[0];
    }
    else
      return 0;  //last element cant be fit into the bag so DONT add its value else return zero
  }
