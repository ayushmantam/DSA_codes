...
public:
		...
		while(!q.empty()) {
			...
			for(auto nbr: adjList[frontNode]) {
				if(!visited[nbr]) {
					q.push(nbr);
					visited[nbr] = true;
					parent[nbr]=frontNode;     //nbr key hai  frontnodes uski value hogi
				}
				if(visited[nbr] && nbr != parent[frontNode]) {      //c
						//cycle present
						return true;
				}
			}
	...
	bool checkCyclicByBfs(int src,unordered_map<int,bool>&visited){
		unordered_map<int,int>parent;
		queue<int>q;
		q.push(src);
		visited[src];
	...
	bool checkCyclicByDfs(int src,unordered_map<int,bool>&visited,int parent){
		visited[true];
		
		for(auto nbr: adjList[src]){
			if(!visited[nbr]){
				bool checkAageKaAns=checkCyclicByDfs(nbr,visited,src);
				if(checkAageKaAns==true){
					return true;
				}
			}
...
unordered_map<int,bool> dfsVisited) {
	...
	bool checkCyclicByDfsForDirectedG(int src,unordered_map<int,bool>&visited,unordered_&dfsvisited){
		visited[src]=true;
		dfsvisited[src]=true;

		for(auto nbr:adjList[src]){
			if(!visited[nbr]){
				bool checkAageKaAns=checkCyclicDirectedGraphUsingDfs(src,visited,dfsvisited);
				if(checkAageKaAns=true){
					return true;
				}
			}
